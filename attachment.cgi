...
sub view {
    ...
    if ( $contenttype =~ /^image\/jpeg$/ && $attachment->data =~ /^\xff\xd8/ ) {
        $disposition = "inline";
    } elsif ( $contenttype =~ /^image\/gif$/ && $attachment->data =~ /^GIF8[79]/ ) {
        $disposition = "inline";
    } elsif ( $contenttype =~ /^image\/png$/ && $attachment->data =~ /^\x89\x50\x4e\x47\x0d\x0a\x1a\x0a/ ) {
        $disposition = "inline";
    }
